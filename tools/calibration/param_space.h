#pragma once
// ─── Parameter space ─────────────────────────────────────────────────────────
// Defines which ArucoConfig fields to optimise and their search bounds.
...
struct ParamSpec {
    const char* key;      // JSON key — used in progress output
    double      lo, hi;   // search bounds (raw value, not normalised)
    bool        logScale; // true → CMA-ES works in log space (for wide-range params)
};

// ─── Search space definition ──────────────────────────────────────────────────
// Bounds should match the physical range that makes sense for the camera setup.
// Narrow them after a first broad sweep to speed up subsequent runs.
...
static const ParamSpec kParams[] = {
    ...
    { "error_corr",      0.50,    0.95, false },
    { "min_otsu_stddev", 2.0,    12.0,  false },
    { "clahe_clip",      0.5,     5.0,  false },
    { "kf_proc_vel",     0.001,   0.10, true  },  // log: spans two orders of magnitude
...
// ArucoConfig → normalised [0,1]^kNParams vector for the optimizer
inline std::vector<double> encode(const ArucoConfig& c) {
    return {
...
// Normalised [0,1]^kNParams vector → ArucoConfig (overlaid on base; base provides
// all non-optimised fields such as camera settings and dictionary ID)
inline ArucoConfig decode(const std::vector<double>& x, ArucoConfig cfg) {
    cfg.threshC       = (float)fromNorm(x[0],  kParams[0]);
    cfg.winMax        = (int)std::round(fromNorm(x[1], kParams[1]));
    if (cfg.winMax % 2 == 0) cfg.winMax++;           // adaptive window must be odd
    ...
    // winStep must fit within the window range
    cfg.winStep = std::min(cfg.winStep, std::max(1, (cfg.winMax - cfg.winMin) / 2));
    return cfg;
}

// ─── Detector factory ─────────────────────────────────────────────────────────
// Build a detector directly from a config — used by objectives to evaluate
// candidates without instantiating a full ArucoTracker.

inline cv::aruco::ArucoDetector makeDetector(const ArucoConfig& cfg) {
...
// Write a complete ArucoConfig back to JSON so the result can be loaded by
// ArucoConfig::fromFile().  Overwrites the file at path.

inline void writeConfig(const ArucoConfig& cfg, const std::string& path) {
