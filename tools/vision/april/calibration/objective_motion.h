#pragma once
// ─── MotionObjective ─────────────────────────────────────────────────────────
// Captures N frames while robots are moving, then evaluates any candidate
// AprilConfig offline against that replay dataset.
//
...
struct MotionObjective : IObjective {
    ...
    bool capture(int camIdx, int nFrames, const AprilConfig& baseCfg,
        ...
        printf("[april-calib] Drive robots at typical operating speed.\n");
        printf("[april-calib] Capture starts in: ");
        for (int t = 3; t >= 1; --t) {
