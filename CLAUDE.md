...
The heading controller is `circle_demo.cpp`'s orbit controller (yaw feedforward + PD, `MAX_TURN_RATE` slew limit); only the source of the tangential speed setpoint differs — per robot from the model, rather than one global orbit rate. Fixes to that control law should probably land in both. Its inputs are circle_demo's too, and deliberately so: the **raw** pose yaw for the heading error and a sample-and-held `RateEstimator` for the D-term. An earlier version low-passed the yaw with a 0.5 s time constant first, which on a circle — where the true heading rotates continuously at `v/R` — lags by about `tau·v/R` and hands the P-term a standing error (~9° at 100 mm/s on a 300 mm ring) it steers out of a robot that was already pointing the right way. Smooth the *rate*, never the heading.
...
The `--bridge` page's two buffering fields are injected by `tools/vision/car_following_bridge.js`, not added to the vendored HTML — the same rule as the reporting script. `applyParams()` picks them up as `buffer-b`/`buffer-id` alongside the scraped NetLogo widget values; this is still UI parameters over the HTTP bridge, never `MSG_*` frames, so the "keep it at three" rule is unaffected.
